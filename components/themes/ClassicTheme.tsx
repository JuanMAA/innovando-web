...
export default function ClassicTheme({ business, photoRefs, reviews, t }: Props) {
    ...
    <div className="min-h-screen bg-white">
            ...
            <div className="flex flex-wrap items-center gap-4">
              {business.rating && (
                <div className="flex items-center gap-2 bg-white/15 backdrop-blur-sm rounded-full px-3 py-1.5">
                  ...
                  <span className="text-white/70 text-sm">({business.num_reviews} {t.labels.reviews})</span>
                </div>
              )}
              {city && <div className="flex items-center gap-1.5 text-white/80 text-sm"><MapPin className="w-4 h-4" />{city}</div>}
            </div>
          </div>
        </div>
      </div>
      ...
      {galleryRefs.length > 0 && (
        <div id="galeria" className="bg-gray-50 py-12">
          <div className="mx-auto max-w-6xl px-6">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">{t.sections.gallery}</h2>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {galleryRefs.map((ref, i) => (
                <img key={i} src={photoUrl(ref)} alt={`${t.sections.gallery} ${i + 2}`} className="w-full aspect-[4/3] object-cover rounded-xl" />
              ))}
            </div>
          </div>
        </div>
      ...
      <div id="contacto" className="py-14 bg-white">
        <div className="mx-auto max-w-6xl px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-4">{t.sections.about} {business.name}</h2>
              <p className="text-gray-600 leading-relaxed text-base mb-6">
                {business.name} · {city}.
                {business.rating && business.rating >= 4 ? ` ${t.labels.excellent}.` : ''}
              </p>
              {business.address && (
            ...
            <div className="bg-gray-50 rounded-2xl p-6 border border-gray-100">
              <h3 className="text-lg font-bold text-gray-900 mb-5">{t.sections.contact}</h3>
              <div className="flex flex-col gap-3">
                ...
                <a href="https://innovando.cl/contacto" className="inline-flex items-center justify-center gap-2 rounded-xl bg-gray-900 px-4 py-3.5 text-sm font-bold text-white hover:bg-gray-800 transition-colors">
                  {t.actions.reserve}<ChevronRight className="w-4 h-4" />
                </a>
              </div>
            </div>
          </div>
        ...
        <div id="resenas" className="py-14 bg-gray-50 border-t border-gray-100">
          <div className="mx-auto max-w-6xl px-6">
            <div className="flex items-end gap-4 mb-8">
              <h2 className="text-2xl font-bold text-gray-900">{t.sections.reviews}</h2>
              {business.rating && (
                <div className="flex items-center gap-2 mb-0.5">
                  <Stars rating={business.rating} />
                  <span className="text-gray-600 font-semibold">{business.rating.toFixed(1)}</span>
                  <span className="text-gray-400 text-sm">· {business.num_reviews} {t.labels.reviews}</span>
                </div>
              )}
            </div>
      ...
      <footer className="bg-gray-950 py-6">
        <div className="mx-auto max-w-6xl px-6 flex items-center justify-between">
          ...
          <p className="text-xs text-gray-500">{t.labels.createdBy}{' '}
