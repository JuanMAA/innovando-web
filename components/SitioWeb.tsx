...
function FloatingWhatsApp({ whatsapp }: { whatsapp: string }) {
  ...
  return (
    <a
      ...
      aria-label="WhatsApp"
      className="fixed bottom-20 right-5 z-50 w-14 h-14 rounded-full shadow-2xl flex items-center justify-center transition-transform hover:scale-110 active:scale-95"
...
export default function SitioWeb({ style = 'classic', lang = 'es', ...rest }: Props) {
  ...
  return (
    <>
      ...
      {rest.business.whatsapp && (
        <FloatingWhatsApp whatsapp={rest.business.whatsapp} />
      )}
    </>
  )
}
